"""mitre atlas framework seed

Revision ID: 0161_mitre_atlas_seed
Revises: 0160_g7_hiroshima_seed
...
from app.services.framework_seed_data_stream_a6 import ATLAS_NIST_AIRMF_MAPPINGS, ATLAS_OBLIGATIONS, ATLAS_QUESTIONS, ATLAS_SECTIONS

revision: str = "0161_mitre_atlas_seed"
down_revision: str | None = "0160_g7_hiroshima_seed"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def _seed_atlas(bind: sa.Connection) -> None:
    frameworks = sa.table(
        "frameworks",
        ...
        bind.execute(
            frameworks.insert().values(
                ...
                description=(
                    ...
                    "Knowledge base of adversarial ML attack tactics and techniques observed in real AI systems."
                ),
...
def _seed_mappings(bind: sa.Connection) -> None:
    obligations = sa.table(
        "obligations",
        sa.column("id", sa.Uuid()),
        sa.column("reference_code", sa.String()),
...
def downgrade() -> None:
    ...
    framework_id = bind.execute(sa.select(frameworks.c.id).where(frameworks.c.name == "MITRE ATLAS")).scalar_one_or_none()

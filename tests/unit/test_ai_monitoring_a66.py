...
def test_a66_monitoring_mode_b(client):
    ...
    # Within threshold (above): value stays below the threshold (healthy).
    within = client.post(
    ...
    # Breach (above): value meets/exceeds the threshold.
    ...
    # Breach (below): value meets/falls below the threshold.
    ...
    breach_below = client.post(
        f"{MONITORING_BASE}/readings",
        headers=org["org_headers"],
        json={"config_id": below_config_id, "value": 0.85, "source_tool": "manual_runner"},
    )

...
from app.models.shadow_ai_signature import ShadowAISignatureRegistry, ShadowAITelemetryEvent
...
class ShadowAIConfidenceEngine:
    """Weighted signal aggregation algorithm — Core Patent Claim 1."""

    @staticmethod
    def compute_score(
        signature: ShadowAISignatureRegistry,
        events: list[ShadowAITelemetryEvent],
    ) -> tuple[float, dict]:
        ...
        events_by_signal: dict[str, list[ShadowAITelemetryEvent]] = {}
        for event in events:
        ...
        for signal_type in ("endpoint_match", "identity_match", "volume_match", "keyword_match"):
            ...
            if signal_type == "keyword_match":
                score = ShadowAIConfidenceEngine._compute_keyword_signal_score(
                    signal_events, keyword_patterns
                )
            elif signal_type == "endpoint_match":
                score = ShadowAIConfidenceEngine._compute_endpoint_signal_score(
                ...
                score = ShadowAIConfidenceEngine._compute_identity_signal_score(
                    signal_events, oauth_patterns
                )
            elif signal_type == "volume_match":
                score = ShadowAIConfidenceEngine._compute_volume_signal_score(

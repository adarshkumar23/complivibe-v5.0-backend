...
def _result_payload(row: ExportControlCheck) -> dict[str, Any]:
    return {
        "id": str(row.id),
...
def screen_vendor_export_control(
    ...
    try:
        row = ExportControlScreeningService(db).screen(
            organization,
            vendor,
...
def get_vendor_export_control(
    ...
    row = ExportControlScreeningService(db).latest_check(organization.id, vendor_id)
    if row is None:
...
def get_vendor_export_control_history(
    ...
    VendorService(db).require_vendor_in_org(organization.id, vendor_id)
    rows = ExportControlScreeningService(db).list_checks(organization.id, vendor_id)
